...
- 节点详情包含：
  ...
  - 推荐项目
  - 推荐资源
- 支持新增、编辑、删除节点，节点删除需要二次点击确认，降低误删风险。
- 删除节点需要二次确认，避免误删路线结构。
...
- 账号入口已接入 Supabase Auth 浏览器客户端；配置公开环境变量后可用邮箱登录/注册，未配置时会给出明确提醒。
...
- 上传页支持：
  ...
  - 服务端解析 PDF 文本
  - 服务端解析 XLSX 工作表
  - 上传页文案会提示 PDF / XLSX 已支持内容抽取
  - 导入符合结构的 JSON 路线
  - 导入猫扑本地路线迁移包，一次恢复多条本地路线
...
- 分享路线时会记录到本地分享历史，产品自检面板会展示分享记录数，为后续数据库短链 `shareId` 做准备。
...
1. **云端路线保存**
   ...
   接入 Supabase 路线表和 RLS，把路线、节点、收藏和学习状态从本地存储迁到云端。
...
3. **账号体验补全**
   ...
   补 OAuth、找回密码、邮箱模板、服务端会话保护，以及登录后自动迁移本地路线。

4. **共享限流和额度面板**

   当前已有单实例内存限流；多实例部署时接 Redis / Upstash，并提供后台调用统计。
...
6. **文件解析升级**
   ...
   优先实现 Markdown/TXT 完整解析，再继续增强 PDF、XLSX 和 GitHub 仓库抓取。
